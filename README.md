...
\- \*\*91 answerable\*\*: LLM-generated Q\&A from randomly sampled chunks, human-reviewed. Each maps to a `gold\_chunk\_id` for recall/MRR.
...
| Chunking | 450 tokens, 80 overlap, heading-aware | bge-small has max\_seq=512; heading-aware keeps chunks topically coherent |
...
| Lexical | rank\_bm25 | Custom tokenizer preserves `kube-proxy`, `v1.28`, `ConfigMap` |
...
python -m eval.run\_eval --tag local
...
│   ├── run\_eval.py        eval loop → JSONL + summary

│   ├── check\_thresholds.py  CI gate
